...
{
	if((lpRect->bottom==bottom) && (lpRect->top==top) && (lpRect->left==left) && (lpRect->right==right))
		return TRUE;
	return FALSE;
}
...
BOOL CRect::UnionRect(LPRECT lpRect1, LPRECT lpRect2)
{
	...
	{
		left = lpRect2->left;right = lpRect2->right;
		top = lpRect2->top;bottom = lpRect2->bottom;
		return TRUE;
	}
	if(((CRect*)lpRect2)->IsRectNull())
	{
		left = lpRect1->left;right = lpRect1->right;
		top = lpRect1->top;bottom = lpRect1->bottom;
		return TRUE;
	}
...
{
	if((top==rect.top) && (bottom==rect.bottom) && (right==rect.right) && (left==rect.left))
		return TRUE;
	return FALSE;
}
...
CRect CRect::operator&(const RECT& rect2) const
{
	...
	if(rect2.left>rect2.right)
		rect1.left=rect1.right=0;
	if(((rect1.top<rect2.bottom) && (rect1.left<rect2.right)) || ((rect2.top<rect1.bottom) && (rect2.left<rect1.right))) 
	{
...
BOOL CFile::Open(LPCTSTR lpszFileName, UINT nOpenFlags)
{
	...
	mystrcpy(m_szFileName, lpszFileName);
#ifdef _LINUX
	int i;
